...
class AutoEncoder(pl.LightningModule):
    def __init__(self, number_of_users, number_of_movies, encoding_size, all_data):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(in_features=number_of_movies, out_features=1024),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Linear(in_features=32, out_features=encoding_size),
            nn.ReLU()
        )

        # Decoder part of the Autoencoder
        self.decoder = nn.Sequential(
            nn.Linear(in_features=encoding_size, out_features=32),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Linear(in_features=1024, out_features=number_of_movies),
            nn.ReLU()
        )
    ...
    def reconstruct_full_matrix(self):
        reconstructed_matrix = torch.zeros((hyper_params['number_of_users'], hyper_params['number_of_movies']))

        for i in range(0, hyper_params['number_of_users'], hyper_params['batch_size']):
            upper_bound = min(i + hyper_params['batch_size'], hyper_params['number_of_users'])
