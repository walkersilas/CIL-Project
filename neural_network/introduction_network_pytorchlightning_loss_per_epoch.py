...
class NCF(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        users, movies, ratings = batchintroduction_network_pytorchlightning.py

        predictions = self(users, movies)
        squared_error = (predictions.cpu() - ratings.cpu()) ** 2
        return squared_error

    def validation_epoch_end(self, validation_outputs):
        all_squared_errors = torch.cat(validation_outputs)
        mse = torch.mean(all_squared_errors)
        rmse = math.sqrt(mse)

        self.logger.experiment.log_metric('Reconstruction RMSE per Epoch', rmse, step=self.current_epoch)
