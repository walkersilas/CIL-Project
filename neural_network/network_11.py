...
hyper_params = {
    'batch_size': 1024,
    'num_epochs': 25,
    'mlp_user_embedding_size': 10000,
    'mlp_movie_embedding_size': 1000,
    'gmf_embedding_size': 1000,
    'learning_rate': 1e-3,
    'train_size': 0.9,
    'dropout': 0.5,
...
class NCF(pl.LightningModule):
    def __init__(self, number_of_users, number_of_movies, mlp_user_embedding_size,
                 mlp_movie_embedding_size, gmf_embedding_size):
        super().__init__()
        ...
        self.mlp_embedding_layer_users = nn.Linear(number_of_users, mlp_user_embedding_size)
        self.mlp_embedding_layer_movies = nn.Linear(number_of_movies, mlp_movie_embedding_size)

        self.gmf_embedding_layer_users = nn.Linear(number_of_users, gmf_embedding_size)
        self.gmf_embedding_layer_movies = nn.Linear(number_of_movies, gmf_embedding_size)

        # Multilayer Perceptron Network
        self.mlp = nn.Sequential(
            nn.Dropout(p=hyper_params['dropout']),
            nn.Linear(in_features=mlp_user_embedding_size + mlp_movie_embedding_size, out_features=64),
            nn.ReLU(),
            nn.Dropout(p=hyper_params['dropout']),
        ...
        self.prediction_layer = nn.Linear(in_features=8 + gmf_embedding_size, out_features=1)
    ...
    def forward(self, users, movies):
        ...
        # Compute Multilayer Perceptron part of Network
        mlp_users_embedding = self.mlp_embedding_layer_users(users_one_hot)
        mlp_movies_embedding = self.mlp_embedding_layer_movies(movies_one_hot)

        mlp_concat = torch.cat([mlp_users_embedding, mlp_movies_embedding], dim=1)
        mlp_output = self.mlp(mlp_concat)

        # Compute Matrix Factorization part of Network
        gmf_users_embedding = self.gmf_embedding_layer_users(users_one_hot)
        gmf_movies_embedding = self.gmf_embedding_layer_movies(movies_one_hot)

        gmf_output = self.gmf(gmf_users_embedding, gmf_movies_embedding)

        # Concatenate output of MLP and GMF and use output layer to make prediction
        prediction = self.prediction_layer(torch.cat([mlp_output, gmf_output], dim=1))
...
def main():
    ...
    ncf = NCF(hyper_params['number_of_users'],
              ...
              hyper_params['mlp_user_embedding_size'],
              hyper_params['mlp_movie_embedding_size'],
              hyper_params['gmf_embedding_size'])
