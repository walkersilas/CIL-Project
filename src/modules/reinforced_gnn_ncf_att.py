...
class GNN(pl.LightningModule):
    def __init__(self, train_data, val_data, test_data, test_ids, args, laplacian_matrix, config):
        ...
        self.embedding_users = Attention(
            self.number_of_users, self.number_of_movies, self.embedding_size
        )
    ...
    def get_initial_embeddings(self):
        ...
        users_embedding = torch.stack(
            [
                self.embedding_users(tensor(user, device=self.device), movies)
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.args.ensemble_learning:
            predictions_pd = pd.DataFrame({"Id": self.test_ids, "Prediction": predictions})
            file_name = (
                "ensemble/reinforced_gnn_ncf/predictions" + str(self.args.random_seed) + ".csv"
            )
